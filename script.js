...
const DEFAULT_HERO = {
    title: 'Discover great deals\nevery day at Hawa Dennis',
    subtitle: 'Quality fashion and accessories at affordable prices — fast delivery, easy returns.',
    image: 'https://placehold.co/720x500/f7efe7/9a3412?text=Hawa+Dennis',
    images: ['https://placehold.co/720x500/f7efe7/9a3412?text=Hawa+Dennis'],
...
function applyHeroToPage() {
    ...
    try {
        ...
        const images = getHeroImages(hero);
        if (t) t.innerHTML = hero.title ? hero.title.replace(/\bbevery\b/gi, 'every').replace(/\n/g, '<br>') : '';
...
function loadCategoriesFromStorage() {
    try {
        ...
        const parsed = JSON.parse(raw);
        return normalizeCategories(Array.isArray(parsed) && parsed.length ? parsed : DEFAULT_CATEGORIES.slice());
...
function normalizeCategories(categories) {
    ...
    const othersIndex = unique.findIndex(category => category.toLowerCase() === 'others');
    if (othersIndex !== -1) {
        const others = unique.splice(othersIndex, 1)[0];
        unique.push(others);
    }
    return unique;
}
...
function saveCategoriesToStorage(categories) {
    try { localStorage.setItem(CATEGORIES_STORAGE_KEY, JSON.stringify(normalizeCategories(categories))); } catch (e) { }
}
...
function addCategory(name) {
    ...
    const othersIndex = categories.findIndex(category => category.toLowerCase() === 'others');
    if (othersIndex === -1) categories.push(name);
    else categories.splice(othersIndex, 0, name);
...
function saveProductsToStorage(list) {
    ...
    return saveProductsToFirestore(list);
}

function getCachedProducts() {
    try {
        const cached = JSON.parse(localStorage.getItem(PRODUCT_STORAGE_KEY) || '[]');
...
// Start with the last catalog immediately; Firestore refreshes it in the background.
let products = [];
// ======================================================
...
async function saveProductsToFirestore(list) {
    await Promise.all(list.map(saveProductToFirestore));
}
...
async function addProductFromAdmin(payload) {
...
function editProduct(id) {
    ...
    const updatedProduct = { ...p, name, price, description, category, image, stock };
    saveProductToFirestore(updatedProduct).then(() => {
...
// Cart items are requests only; only an admin changes the catalog stock.
...
const DISPLAY_ROTATION_KEY = 'customer_product_rotation_v1';
let customerDisplayOrder = null;
let customerDisplaySignature = '';

function getRecentDisplayIds() {
    try {
        const value = JSON.parse(localStorage.getItem(DISPLAY_ROTATION_KEY) || '[]');
...
function saveRecentDisplayIds(ids) {
    try { localStorage.setItem(DISPLAY_ROTATION_KEY, JSON.stringify(ids.slice(0, 8))); } catch (error) { }
}

// Customer-only rotation: Firebase order stays untouched, while recent top items
// receive a penalty so new and older products both get a fair first-page chance.
function createCustomerDisplayOrder(list) {
    const recentIds = getRecentDisplayIds();
    const recentRank = new Map(recentIds.map((id, index) => [id, index]));
    const ordered = list.map(product => ({
        product,
        score: Math.random() * (recentRank.has(String(product.id)) ? 0.35 : 1) +
            (recentRank.has(String(product.id)) ? recentRank.get(String(product.id)) * 0.02 : 0)
    }));

    ordered.sort((a, b) => a.score - b.score);
    const result = ordered.map(entry => entry.product);
    saveRecentDisplayIds(result.slice(0, 8).map(product => product.id));
    return result;
}

function getCustomerDisplayOrder() {
    if (!container) return products.slice();
    const signature = products.map(product => String(product.id)).sort().join('|');
    if (!customerDisplayOrder || customerDisplaySignature !== signature) {
        customerDisplayOrder = createCustomerDisplayOrder(products);
        customerDisplaySignature = signature;
    }
    return customerDisplayOrder.slice();
}

// Active filters
let currentCategory = 'All';
let currentSearch = new URLSearchParams(window.location.search).get('q')?.trim().toLowerCase() || '';
const initialSearchInput = document.getElementById('search-bar');
if (initialSearchInput && currentSearch) initialSearchInput.value = currentSearch;

function isDealProduct(product) {
    return product.category === 'Deals' || product.isDeal === true || product.deal === true ||
        Number(product.discount) > 0 || Number(product.salePrice) > 0 || Number(product.compareAtPrice) > Number(product.price || 0);
}
...
function showDeals() {
    ...
    if (heading) heading.textContent = 'Deals for You';
    const productsSection = document.getElementById('products') || document.getElementById('all-products');
...
function renderHomepageHighlights() {
    const categoryContainer = document.getElementById('homepage-categories');
    if (categoryContainer) {
        const categoryNames = loadCategoriesFromStorage();
    ...
    if (featuredContainer) {
        const featured = products.filter(isDealProduct).length ? products.filter(isDealProduct) : products.slice(0, 8);
        featuredContainer.innerHTML = featured.slice(0, 8).map(product => {
            ...
            return `<article class="featured-card">
                ...
                <button class="featured-add" type="button" ${available <= 0 ? 'disabled' : ''} onclick="addToCart('${product.id}')">Add to cart</button>
            </article>`;
        }).join('');
    ...
    try {
        ...
        cart = cart.map(item => {
            if (!item.id) {
                ...
                if (found) return { id: found.id, name: found.name, price: found.price, image: found.image || item.image || '', description: found.description || item.description || '', qty: item.qty || 1 };
            }
            if (!item.image) {
...
function addToCart(productId) {
    ...
    if (found) {
        ...
        cart.push({ id: prod.id, name: prod.name, price: prod.price, image: prod.image || '', description: prod.description || '', qty: 1 });
    }
...
function changeQty(index, delta) {
    ...
    if (delta > 0) {
        ...
        if (item.qty >= avail) { showToast('No more stock'); return; }
    }
    item.qty += delta;
    if (item.qty <= 0) {
        cart.splice(index, 1);
...
function addMultipleToCart(productId, qty) {
    ...
    // Add only up to the catalog stock; cart contents do not reserve stock.
    const available = getAvailableStock(productId);
    const prod = products.find(x => String(x.id) === String(productId));
    if (!prod) return;

    const found = cart.find(i => i.id === prod.id);
    const remainingForCart = Math.max(0, available - (found ? found.qty : 0));
    const toAdd = Math.min(qty, remainingForCart);
    if (toAdd <= 0) { showToast('This item is out of stock'); return; }
    if (found) {
        found.qty += toAdd;
    } else {
        cart.push({ id: prod.id, name: prod.name, price: prod.price, image: prod.image || '', description: prod.description || '', qty: toAdd });
    }
...
function updateCustomerHeaderOffset() {
    if (document.querySelector('.admin-main')) return;
    const header = document.querySelector('.site-header');
    if (header) document.documentElement.style.setProperty('--customer-header-height', `${header.offsetHeight}px`);
}

updateCustomerHeaderOffset();
window.addEventListener('resize', updateCustomerHeaderOffset);
...
async function initializeStore() {
    ...
    // Keep the catalog live without blocking the first paint on network latency.
    await new Promise(resolve => {
        ...
        db.collection('products').onSnapshot(snapshot => {
            ...
            console.error('Firestore product listener error:', error);
            if (firstSnapshot) {
                firstSnapshot = false;
                resolve();
            }
...
window.addEventListener('storage', (e) => {
    ...
    if (e.key === CATEGORIES_STORAGE_KEY) renderHomepageHighlights();
});
...
function filterCategory(category) {
    ...
    const heading = document.getElementById('products-heading');
    if (heading) heading.textContent = category === 'All' ? 'Top Picks For You' : `${category} products`;
    applyFilters();
...
function submitProductSearch() {
    ...
    if (!container && currentSearch) {
        window.location.href = `index.html?q=${encodeURIComponent(currentSearch)}`;
        return;
    }
    const productsSection = document.getElementById('products');
    if (productsSection && currentSearch) productsSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function applyFilters() {
    // Rotate the complete catalog before applying filters, so pagination or a
    // narrow category cannot permanently hide older products.
    let filtered = getCustomerDisplayOrder();

    if (currentCategory === 'Deals') {
        filtered = filtered.filter(isDealProduct);
